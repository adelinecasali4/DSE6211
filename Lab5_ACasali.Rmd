...
Data preprocessing, neural network model training, and ROC + Calibration curve creation
```{r}
...
1) In the ROC curve above, the tpr and fpr associated with the threshold value of 0.3 are 0.9017288 and 0.537963, respectively. 
```{r}
ggplot() + 
  geom_line(data = roc_data, 
            aes(x = fpr, y = tpr, color = threshold), linewidth = 2) + 
  ...
  geom_text(data = roc_data[seq(1, 101, 10), ], 
            aes(x = fpr, y = tpr, label = threshold, hjust = 1.2, vjust = -0.2))
over_threshold <- test_set[test_set$p_prob >= 0.3, ]
fpr <- sum(over_threshold$lodgepole_pine==0)/sum(test_set$lodgepole_pine==0)
fpr
tpr <- sum(over_threshold$lodgepole_pine==1)/sum(test_set$lodgepole_pine==1)
tpr
...
2) In this calibration curve, the predicted probabilities in the interval (0.2, 0.3) are under-confident probabilities, as the point associated with the mid-point (0.25) lies above the diagonal line. 
```{r}
ggplot(data = calibration_data, aes(x = bin_midpoint, y = observed_event_percentage)) +
  geom_line(linewidth = 1) +
  geom_abline(intercept = 0, slope = 1, lty = 2) +
...
3) Below are the ROC and calibration curves. 
```{r}
